...
public class TestAlgsPlus {
    ...
    private static final String PATH_ANNOT_FOLDER= "/crypto/annot/";
    ...
    private Method checkAnnotatedMethod(Class c, Class annotatedClass){

        Method[] methods = c.getMethods();
        for(Method method : methods){
            if(method.getParameterCount() == 1 && method.getParameterTypes()[0].
                    getSimpleName().equals("String")) {
                if(method.getAnnotation(annotatedClass)!=null) {
                    return method;
                }
            }
    ...
    public void checkAlgos(Path path) throws MalformedURLException, ClassNotFoundException, NoSuchMethodException,
        ...
        for (File file : files) {
            if (file.getName().endsWith(".class")) {
                ...
                /*now in case the class doesn't have a method that start with "enc" or "dec" we go to check if the class
                  has method that have annotation "@Encrypt" or "@Decrypt" */
                if(hasEncMethod==null){
                   hasEncMethod= this.checkAnnotatedMethod(c,Encrypt.class);
                }
                if(hasDecMethod==null){
                    hasDecMethod=this.checkAnnotatedMethod(c,Decrypt.class);
                }
